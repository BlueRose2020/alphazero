...
class QuickModel(BaseModel):
    ...
    def __init__(self) -> None:
        super().__init__()
        if USE_HISTORY:
            in_channels = GAME_STATE_DIM * HISTORY_LEN + 1
        else:
            in_channels = GAME_STATE_DIM + 1
        ...
        if USE_RESNET_BLOCK:
            self._append_shared_layers_with_resnet(in_channels=in_channels)
        else:
            self._append_shared_layers_without_resnet(in_channels=in_channels)
    ...
    def _append_policy_head(self) -> None:
        ...
        # 池化与展平，确保线性层输入维度正确
        self.layers_dict["policy_head"].append(nn.AdaptiveAvgPool2d((1, 1)))
        ...
        if POLICY_LINEAR_NUM >= 2:
            add_policy_linear(last_conv_out_channels, policy_linear_list[0])
            ...
            add_policy_linear(last_conv_out_channels, NUM_ACTION, use_activation=False)
    ...
    def _append_value_head(self) -> None:
        ...
        # 池化与展平，确保线性层输入维度正确
        self.layers_dict["value_head"].append(nn.AdaptiveAvgPool2d((1, 1)))
        ...
        if VALUE_LINEAR_NUM >= 2:
            add_value_linear(last_conv_out_channels, value_linear_list[0])
            ...
            add_value_linear(last_conv_out_channels, 1, use_activation=False)
    ...
    def _append_shared_layers_with_resnet(self, in_channels: int) -> None:
        ...
        # 添加 ResNet 块（数量由配置 RESNET_NUM 控制）
