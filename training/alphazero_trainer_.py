...
def _training_worker(
    ...
    try:
        ...
        if os.path.exists(os.path.join(model_save_dir, "last_optim.pth")):
            try:
                trainer.optim.load_state_dict(
                    torch.load(os.path.join(model_save_dir, "last_optim.pth"))
                )
                logger.info(f"已加载之前的优化器状态: {model_save_dir}/last_optim.pth")
            except Exception as e:
...
class AlphaZeroTrainer:
    ...
    def train(
        ...
        if USE_MULTIPROCESSING and isinstance(self.exp, SharedRingBufferExperiencePool):
        ...
        if not USE_MULTIPROCESSING:
            _save_optimizer(self.trainer.optim, self._model_save_dir)

        logger.info("保存经验池")
        self.exp.save(self._get_experience_path())
        logger.info("经验池保存完成")
    ...
    def _load_previous_state(self) -> None:
        ...
        # 加载之前的优化器状态
        if os.path.exists(os.path.join(self._model_save_dir, "last_optim.pth")):
            try:
                self.trainer.optim.load_state_dict(
                    torch.load(os.path.join(self._model_save_dir, "last_optim.pth"))
                )
                logger.info(
                    f"已加载之前的优化器状态: {self._model_save_dir}/last_optim.pth"
                )
            except Exception as e:
                logger.warning(f"加载优化器状态失败: {e}")
