...
TRAIN_NUM_SIMULATION = 80 # 每个样本进行多少次蒙特卡洛树搜索模拟，模拟越多训练越慢但效果越好
...
SELF_PLAY_UPDATE_MODEL_FREQUENCY = 10  # 每多少轮自对弈后更新一次模型(单个进程)
...
_SELF_PLAY_WORKER_NUM = 26
# 自对弈进程数量，可以为 "auto"，会自动设置为CPU核心数减2
# (一个进程用于训练，另一个进程用于避免死机，当自对弈数量少
# 于CPU核心数时，会自动设置为自对弈数量）
# 你可以设置为一个固定的整数
# ps:若使用auto有可能出现资源不足的情况，减小自对弈数量或直
# 接设置为一个固定的较小整数或者调整系统虚拟内存可以解决该问题
