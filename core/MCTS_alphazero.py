...
class MCTS:
        ...
        for _ in range(num_simulation):
            ...
            if node.is_terminal():
                ...
                with torch.inference_mode():
                    policy, value = model(nn_state.to(DEVICE))
    ...
    def _apply_virtual_loss_path(self, path: list[MCTSNode]) -> None:
        for node in path[1:]:
            node.virtual_loss += VIRTUAL_LOSS

    def _revert_virtual_loss(self, path: list[MCTSNode]) -> None:
        for node in path[1:]:
            node.virtual_loss = max(0.0, node.virtual_loss - VIRTUAL_LOSS)
    ...
    def _node2nn_state(self, node: MCTSNode) -> NNState:
        ...
        # 不需要额外 clone，直接移动到 DEVICE 即可以减少拷贝开销
        # 保持在 CPU 上，批量时再一次性移动到 DEVICE，减少多次主机->设备拷贝
